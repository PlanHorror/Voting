...
import { SignerService } from "@/services/signer.service"; // Add SignerService import
...
export default function CreateVoteSessionPage() {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  // Add state for signers
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    endDate: getTomorrow(),
    supervisorId: "", // This would typically come from context/state in a real app
    signerId: "", // This will be selected from the fetched signers
  });
    ...
    try {
      ...
      if (error instanceof AxiosError) {
        ...
        if (error.response && error.response.status === 400) {
          ...
          toast.error("You're not authorized to create vote sessions.");
          router.push("/login");
        } else {
          toast.error("Failed to create vote session. Please try again.");
        }
      }
    } finally {
